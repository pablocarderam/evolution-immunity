...
print(npop, ' populations...')
print('Completely connected...' if completely_connected else 'Partially connected...')
...
if do_intervention is True:
    print('Intervention enabled...')
    intervine_pops = ['A','B','D','E']  # intervine the periferic populations
...
# Do intervention if desired
t_d = 3000 # time at which drug is applied
if do_intervention is True:
    for intpop in range(len(intervine_pops)): # for each population
        model.newIntervention( t_d, 'treatHosts', [ 'pop' + intervine_pops[intpop], 1, [re_seq] ] )


# Run and save
# output = model.run(0,100,time_sampling=0)
t_f = t_d+3000 # final timepoint

model.run(0,t_f,time_sampling=100, host_sampling=0, vector_sampling=0)
...
graph = model.populationsPlot( # Plot infected hosts per population over time.
    'tests/5pops_sims_immunity_loss{}/{}/metapopulations_migration_5pops.png'.format(intername[interfolder],savename[saveas]), 
    data,
...
graph_composition = model.compositionPlot(
    'tests/5pops_sims_immunity_loss{}/{}/pathogen_comp_metapopulations_migration_5pops.png'.format(intername[interfolder],savename[saveas]), 
    data,
...
for i, pop in enumerate(n_populations):
    graph_composition = model.compositionPlot(
    'tests/5pops_sims_immunity_loss{}/{}/individual_pop/pathogen_comp_metapopulations_migration_pop{}.png'.format(intername[interfolder],savename[saveas], pop), 
    data,
    # composition_dataframe=comp_dat,
    populations=['pop' + pop],
