...
path_save = 'tentative_figs/fig3_immunity_effect_over_evolution/immunity_effect_over_evolution_weights_dist_fitvalley'
...
w1 = [0.4,0.25,0.15,0.1,0.05,0.02,0.02,0.01]  # decaying weights
w2 = [0.01,0.02,0.02,0.05,0.1,0.15,0.25,0.4]  # increasing weights
w3 = [0.005,0.02,0.075,0.4,0.4,0.075,0.02,0.005]  # bell weights
w4 = [0.015,0.035,0.15,0.3,0.3,0.15,0.035,0.015]  # bell soft
w5 = [0.4,0.075,0.02,0.005,0.005,0.02,0.075,0.4]  # U weights (ultra high)
w6 = [0.4,0.075,0.02,0.005,0.005,0.02,0.075,0.4]  # U weights (ultra soft)
...
for key, elem in exps.items():    
    def myImmuneWeights(genome,immune_seq,weights=elem):
        if len(weights) == 1:
            ...
            # Element matching: any mismatches between pathogen genome and immune
            # memory generate no immunity. Only perfect matches generate 100%
            # immunity.
    ...
    graph_populations = model.populationsPlot( # Plot infected hosts per population over time.
        ...
        data,
        num_top_populations=1, # plot all 2 populations
